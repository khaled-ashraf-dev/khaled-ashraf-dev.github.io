...
function addRequiredQuests() {
  ...
  for (const i of selectedQuestsIds) {
    ...
    for (const j of jsonData[i].required_quests) {
      ...
      <input sub-quest-id=${i} type="checkbox" id="required-quest-checkbox-${j}" class="sub-checkbox">
...
// save presets
const logButton = document.getElementById("save-btn");
function logCheckboxStatus() {
  ...
  console.log("Checkbox status saved to localStorage:", status);
}

logButton.addEventListener("click", logCheckboxStatus);